...
def train_ppo_atari(env_id="ALE/Pong-v5", total_timesteps=100000, save_dir="models"):
    ...
    # Sử dụng make_atari_env để tạo môi trường chuẩn (bao gồm wrap các tính năng chuẩn của Atari)
    # Lưu ý: Nếu muốn dùng OOD Wrapper của chúng ta sau này, ta thu thập data trên single env
    # nên ở đây ta huấn luyện trên env chuẩn
    vec_env = make_atari_env(env_id, n_envs=4, seed=0)
    # Xếp chồng 4 frame liên tiếp để giúp mạng học được motion
    vec_env = VecFrameStack(vec_env, n_stack=4)
    
    print(f"Khởi tạo PPO model...")
